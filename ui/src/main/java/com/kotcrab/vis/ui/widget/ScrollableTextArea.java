...
public class ScrollableTextArea extends VisTextArea implements Cullable {
    ...
    private void updateScrollPosition() {
        if (cullingArea == null || !(getParent() instanceof ScrollPane)) return;
        ScrollPane scrollPane = (ScrollPane) getParent();

        if (!cullingArea.contains(getCursorX(), cullingArea.y)) {
            scrollPane.setScrollPercentX(getCursorX() / getWidth());
        }

        if (!cullingArea.contains(cullingArea.x, (getHeight() - getCursorY()))) {
    ...
    public void setText(String str) {
        super.setText(str);
        if (!programmaticChangeEvents) { //changeText WILL NOT be called when programmaticChangeEvents are disabled
            updateScrollLayout();
        }
    }
