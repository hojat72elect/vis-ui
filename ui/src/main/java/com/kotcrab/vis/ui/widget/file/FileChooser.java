...
public class FileChooser extends VisWindow implements FileHistoryCallback {
    ...
    private void init(FileHandle directory) {
        ...
        fileNameSuggestionPopup.setListener(new PopupMenu.PopupMenuListener() {
            ...
            public void activeItemChanged(MenuItem newItem, boolean changedByKeyboard) {
                if (!changedByKeyboard || newItem == null) return;
    ...
    private void rebuildViewModePopupMenu() {
        ...
        for (final ViewMode mode : ViewMode.values()) {
            if (mode.thumbnailMode && !iconProvider.isThumbnailModesSupported()) continue;
    ...
    private void createListeners() {
        addListener(new InputListener() {
            @Override
            public boolean keyDown(InputEvent event, int keycode) {
                if (keycode == Keys.A && UIUtils.ctrl() && !(getChooserStage().getKeyboardFocus() instanceof VisTextField)) {
                    selectAll();
                    return true;
                }
        ...
        for (final File root : roots) {
            DriveCheckerListener listener = new DriveCheckerListener() {
                ...
                public void rootMode(File root, RootMode mode) {
                    if (!driveCheckerListeners.removeValue(this, true)) return;
    ...
    private void rebuildFileList(final boolean stageChanged) {
        ...
        if (!showBusyBarTask.isScheduled()) {
    ...
    private void updateSelectedFileFieldText(boolean ignoreKeyboardFocus) {
        if (!ignoreKeyboardFocus && getChooserStage() != null) {
    ...
    private FileHandle[] listFilteredCurrentDirectory() {
        ...
        int count = 0;
        for (FileHandle file : files) {
            if (!file.isDirectory() && !activeFileTypeRule.accept(file)) continue;
            filtered[count++] = file;
        }
        ...
        fileWatcherThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (fileWatcherThread != null) {
                    ...
                    if (roots.length != lastRoots.length || !Arrays.equals(lastRoots, roots))
                    ...
                    if (lastCurrentDirectory.equals(currentDirectory)) {
                        ...
                        if (lastCurrentFiles.length != currentFiles.length || !Arrays.equals(lastCurrentFiles, currentFiles))
    ...
    public enum ViewMode {
        ...
        public void setupGridGroup(Sizes sizes, GridGroup group) {
            ...
            if (gridSize < 0) {
                throw new IllegalStateException("FileChooser's ViewMode " + this + " has invalid size defined in Sizes. " +
    ...
    public static class DefaultFileFilter implements FileFilter {
        ...
        public boolean accept(File f) {
            ...
            if (chooser.getMode() == Mode.OPEN ? !f.canRead() : !f.canWrite()) return false;
            return ignoreChooserSelectionMode || f.isDirectory() ||
    ...
    public class FileItem extends Table implements Focusable {
        ...
        public FileItem(final FileHandle file, ViewMode viewMode) {
            ...
            boolean shouldShowItemShowCheckBox = showSelectionCheckboxes && (
                    (selectionMode == SelectionMode.FILES_AND_DIRECTORIES)
                            || (selectionMode == SelectionMode.FILES && !metadata.isDirectory())
        ...
        private void addListeners() {
            ...
            addListener(new ClickListener() {
                ...
                public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                    ...
                    if (!handleSelectClick(false)) return false;
        ...
        private boolean handleSelectClick(boolean checkboxClicked) {
            ...
            if (checkboxClicked) {
                if (!multiSelectionEnabled && !selectedItems.contains(FileItem.this, true))
                    deselectAll();
            } else {
                if (!multiSelectionEnabled || (!isMultiSelectKeyPressed() && !isGroupMultiSelectKeyPressed()))
                    deselectAll();
            }
        ...
        private boolean select(boolean deselectIfAlreadySelected) {
            ...
            if (!selectedItems.contains(this, true)) selectedItems.add(this);
            return true;
        }
