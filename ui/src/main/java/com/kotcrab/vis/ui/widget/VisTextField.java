...
public class VisTextField extends Widget implements Disableable, Focusable, BorderOwner {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        // vis
        if (!isDisabled() && !inputValid && style.errorBorder != null)
    ...
    private VisTextField findNextTextField(Array<Actor> actors, VisTextField best, Vector2 bestCoords, Vector2 currentCoords, boolean up) {
        ...
        for (int i = 0, n = actors.size; i < n; i++) {
            ...
            if (actor instanceof VisTextField textField) {
    ...
    public class TextFieldClickListener extends ClickListener {
        ...
        public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
            ...
            if (stage != null) stage.setKeyboardFocus(VisTextField.this);
            if (!readOnly) keyboard.show(true);
            hasSelection = true;
            return true;
        }
        ...
        public boolean keyDown(InputEvent event, int keycode) {
            ...
            if (UIUtils.shift()) {
                if (keycode == Keys.INSERT && !readOnly) paste(clipboard.getContents(), true);
                if (keycode == Keys.FORWARD_DEL && !readOnly) cut(true);
                selection:
                {
                    int temp = cursor;
                    keys:
